...
| BraTS2018 | T2 to FLAIR; FLAIR to T2 | 20 patient volumes in total; 16 patient volumes for training and 4 for testing; the same patient-level split is used for both modalities | central brain crop to 128 x 128 x 128, followed by axial 2D slice extraction; 1,746 slices per modality in total, including 512 test slices |
...
**BraTS2018 split clarification.** The values **1,746** and **512** refer to extracted **2D axial slices**, not patient volumes. Dataset partitioning was performed at the patient level before 2D slice extraction. Specifically, **16 patient volumes were used for training and 4 patient volumes for testing**, and the same patient-level partition was applied to both T2 and FLAIR modalities. All slices from the same patient were retained within the same subset to avoid patient-level information leakage. After preprocessing, each modality contained 1,746 2D slices, of which 512 slices were extracted from the 4 test patients.
