...
| BraTS2018 | T2 to FLAIR; FLAIR to T2 | 20 patient volumes in total; 16 patient volumes for training and 4 for testing; the same patient-level split is used for both modalities | central brain crop to 128 x 128 x 128, followed by axial 2D slice extraction; 1,746 training slices and 512 test slices per modality |
...
**BraTS2018 split clarification.** The values **1,746** and **512** refer to two separately constructed 2D slice sets rather than patient volumes. Dataset partitioning was performed at the patient level before 2D slice extraction. Specifically, **16 patient volumes were used for training and yielded 1,746 training 2D axial slices per modality, whereas 4 patient volumes were used for testing and yielded 512 test 2D axial slices per modality**. The same patient-level partition was applied to both T2 and FLAIR modalities, and all slices from the same patient were retained within the same subset to avoid patient-level information leakage.
